...
def get_testdata():
    testdf = pd.read_csv('test1.csv', header=None)
    testlist = []
...
def calcShannon(dataset):
    # 截取类别的list
    classrow = [example[-1] for example in dataset]
...
def majority(dataset):
    ...
    for kind in classList:
        if kind not in classnums.keys():
            ...
            classnums[kind] += 1
    # 次数:类别
    classnums_t = {v: k for k, v in classnums.items()}
    temp = 0
...
def Choosebestsplitfeat(dataset):
    ...
    for i in range(featuresnum):
        # 截取第i个特征的列(属性列表)
        atrscolumn = [example[i] for example in dataset]
...
def createDT(dataset, labels, classDict):
    ...
    for value in uniqueVal:
        myTree[bestfeaturelabel][value] = \
            createDT(splitDataset(dataset, bestfeatureindex, value)
                     , new_labels, classDict)
    return myTree
...
def getinto(dic, labels_noclass, numberlist):
    if dic in ['是', '否']:  # 假定为二分类，只有yes和no两个类别
        return dic
    else:
